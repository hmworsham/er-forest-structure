# Analysis of 2020 Kueppers et al. East River Forest Inventory Data
# Author: Marshall
# Revised: 03-22-2021
...
fidir <- paste0(getwd(), '/Desktop/RMBL/Projects/Forest_Inventory_Dataset/Output/')
wsdir <- paste0(getwd(), '/Desktop/RMBL/Projects/Watershed_Spatial_Dataset/Source/')
rasdir <- paste0(getwd(), '/Desktop/RMBL/Projects/Watershed_Spatial_Dataset/Output/AOP_Clipped_Topo_Rasters/')
...
# 2021 plot info
siteinfo21 <- read.csv(paste0(wsdir, '/EastRiver_Proposed_Sites_2021.csv'))
colnames(siteinfo21[1]) <- 'Site_Name'
...
##########################################################################
# Correlation matrix and correlation plots for 2020 sites
##########################################################################

# Assign zones based on SFA geographic domains
siteinfo20$Zone <- sapply(strsplit(siteinfo20$Site_Name,"-"), `[`, 1)

# Correlation matrix for topo factors for existing plots 2020
topos <- siteinfo20 %>%
  select(Elevation_m,
         Radiation,
         TWI,
...
topo_cor <- rcorr(as.matrix(topos[,1:6]))

corrplot(as.matrix(topo_cor$r), 
         type = 'upper', 
         order = 'FPC',
         tl.col = 'grey10', 
         tl.srt = 75)

# Correlation plots for topo factors for existing plots 2020
...
upper.panel <- function(x, y){
  points(x,y, pch = 19, col = my_cols[as.factor(topos$Zone)])
}
...
pairs(topos[,1:6], 
      ...
      main = 'Scatter plots and correlation coefficients for pairs of topographical parameters')

##########################################################################
# Correlation matrix and correlation plots for 2021 proposed sites
##########################################################################

# Correlation matrix for topo factors for proposed and existing plots 2021
topos21 <- siteinfo21 %>%
  select(Elevation_m,
         Radiation,
         TWI,
...
topo_cor21 <- rcorr(as.matrix(topos21[,1:6]))

corrplot(as.matrix(topo_cor21$r), 
         type = 'upper', 
         order = 'FPC',
         tl.col = 'grey10', 
         tl.srt = 75)

# Correlation plots for topo factors for proposed and existing plots 2021
upper.panel <- function(x, y, group){
  points(x,y, pch = 19, col = my_cols[as.factor(topos21$Established)])
}
...
##########################################################################
# Correlation matrix and correlation plots for full AOP domain
##########################################################################

# Correlation matrix for topo factors for proposed and existing plots 2021
asp <- raster(paste0(rasdir, 'aop_aspect.tif'))
ele <- raster(paste0(rasdir, 'aop_elevation.tif'))
rad <- raster(paste0(rasdir, 'aop_radiation.tif'))
slo <- raster(paste0(rasdir, 'aop_slope.tif'))
tpi <- raster(paste0(rasdir, 'aop_tpi.tif'))
twi <- raster(paste0(rasdir, 'aop_twi.tif'))

aspv <- as.vector(as.matrix(asp))
elev <- as.vector(as.matrix(ele))
radv <- as.vector(as.matrix(rad))
slov <- as.vector(as.matrix(slo))
tpiv <- as.vector(as.matrix(tpi))
twiv <- as.vector(as.matrix(twi))

aoptopos <- data.frame('Aspect' = aspv, 
                       'Elevation' = elev, 
                       'Radiation' = radv, 
                       'Slope' = slov, 
                       'TPI' = tpiv, 
...
plot(ele, col = terrain.colors(50))
...
plot(tpi, col = viridis::inferno(50))

plot(twi, col = viridis::viridis(50))

aop_topocor <- rcorr(as.matrix(aoptopos))

corrplot(as.matrix(aop_topocor$r), 
         type = 'upper', 
         order = 'FPC',
         tl.col = 'grey10', 
         tl.srt = 75)
...
qplot(allom$Height_Avg_M, allom$DBH_Avg_CM, color = allom$Site_Name)

fitted <- allom %>% 
  nest(-Elevation_m) %>% 
  mutate(
    fit = map(data, ~ nls(Height_Avg_M ~ a * DBH_Avg_CM ^ b, data = ., start = list(a=1,b=0.5))),
  )
...
ggplot(augfit, aes(DBH_Avg_CM, Height_Avg_M, color = as.factor(Elevation_m))) + 
  geom_point() +
  geom_line(aes(y = .fitted)) +
  scale_color_ordinal(option = 'D')

ggplot(augfit, aes(log10(DBH_Avg_CM), log10(Height_Avg_M), color = as.factor(Elevation_m))) + 
  geom_line(aes(y = log10(.fitted))) +
  scale_color_ordinal(option = 'D')

allom_topo <- merge(tidfit, siteinfo20, by = 'Elevation_m') %>%
  filter(term == 'b') %>%
  select(Site_Name,
         estimate,
         std.error,
...
qplot(Elevation_m, estimate, data = allom_topo)
el_b_fit <- lm(estimate ~ Elevation_m, data = allom_topo)
tidy(el_b_fit)
summary(el_b_fit)
predict(el_b_fit)
ggplot(allom_topo, aes(Elevation_m, estimate)) +
  geom_point() +
  geom_line(aes(y = predict(el_b_fit))) + 
...
qmd_topo <- merge(QMD, siteinfo20, by = 'Site_Name')
el_qmd_fit <- lm(QMD ~ Elevation_m, data = qmd_topo)
summary(el_qmd_fit)
...
inv_ca <- scale(inv_ca[4:12]) # standardize variables
rownames(inv_ca) <- inv$Tag_Number
inv_ca <- na.omit(inv_ca) # listwise deletion of missing

#distance <- get_dist(inv_ca)
#head(distance)

wss <- (nrow(inv_ca)-1)*sum(apply(inv_ca,2,var))

for (i in 2:15) wss[i] <- sum(kmeans(inv_ca,
                                     centers=i)$withinss)
plot(1:15, wss, type="b", xlab="Number of Clusters",
     ylab="Within groups sum of squares")

k2 <- kmeans(inv_ca, centers = 6, nstart = 25)

fviz_cluster(k2, data=inv_ca)
inv_ca %>%
  as_tibble() %>%
  mutate(cluster = k2$cluster,
         plot = row.names(inv_ca)) %>%
  ggplot(aes(DBH_Avg_CM, Height_Avg_M, color = factor(cluster), label = plot)) +
  geom_text()
