...
for(j in invids){
  drive_download(
    ...
    path = paste0(fidir, 'Inventory_Files_21-03-10/', j, '.xlsx'),
    overwrite = T
  )
}
...
datadir <- paste0(indir, 'Inventory_Files_21-03-10')
files <- list.files(fidir, full.names = T)
...
treesdir <- paste0(fidir, 'TreeCoords')
trees <- list.files(treesdir, full.names = T)

treesdf <- lapply(trees, function(file){
  data.frame(read.csv(file, colClasses = c("X" = 'double', "Y" = 'double')))
})

alldata <- bind_rows(df)
alltrees <- bind_rows(treesdf)

# Remove erroneous duplicate height or dbh entries from inventory data
alldata <- alldata[-c(2734, # tag 331
                 2746, # tag 342
                 818, # tag 1139
                 954),] # tag 1143
colnames(alltrees)[4] <- 'Tag_Number'

# Filter to 2020 records only
data20 <- alldata %>%
