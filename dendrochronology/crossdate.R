...
# Ingest master core index
di.path <- file.path(dendrodir, 'Dendro_Core_Index.xlsx')
dendroindex <- read_excel(di.path, sheet='Cores', col_types=c('text', 'numeric', 'numeric', 'text', 'numeric', 'text', 'numeric', rep('guess', 17)))
dendroindex <- dendroindex[which(rowSums(is.na(dendroindex))<20),]

# Ingest master site name index
siteindex <- read_excel(di.path, sheet='Sites', col_types=c('numeric', 'text', 'text', 'text', 'numeric', rep('guess', 8)))
...
rw <- lapply(ringwidths, '[', c('Ref_Year', 'Measurement'))

# Create name key from site index
namekey <- unique(data.frame('plotid'=siteindex$Plot, 'siteid'=siteindex$ITRDB_SiteID))
namekey <- namekey[order(namekey$plotid),]
rownames(namekey) <- NULL

# Get namesets for ringwidths
nameset <- lapply(rwfiles, fix.names, namekey)

# Select ringwidths and years for each
for(r in seq(length(rw))){
  #nm <- as.character(unlist(strsplit(rwfiles[r], '/|_'))[13])
  nm <- paste(nameset[[r]][c(3,4)], collapse='')
  colnames(rw[[r]]) <- c('Ref_Year', nm) # Assign series name to meas column
  rw[[r]] <- rw[[r]][rowSums(is.na(rw[[r]])) != ncol(rw[[r]]), ]
  rw[[r]][is.na(rw[[r]])] <- 0
  rw[[r]] <- rw[[r]][2:nrow(rw[[r]]),]
  print(head(rw))
}
...
# Subset cores by site and species
sss <- outer(sites, spps, sss.wrapper)

# Split site/spp subsets by dated and undated cores
datesplit <- lapply(sss, make.rwl)

# Get site and spp id pairs
sitespp.ids <- outer(rownames(sss), colnames(sss), paste)

# Get beginning and end years
year.ranges <- lapply(datesplit, get.year.ranges)
...
runseq <- seq(length(datesplit))
rwl.headers <- lapply(runseq, hdr.wrapper, siteindex, sitespp.ids, year.ranges)


# Write rwl data to Tucson
tucson.wrapper <- function(i, rwls, ids, hdrs) {
  
  rwl.d = rwls[[i]][[1]]
  rwl.ud = rwls[[i]][[2]]
  
  outpath.d = file.path(rwldir, paste(str_replace(ids[[i]], ' ', '_'), 'dated.rwl', sep='_'))
  outpath.ud = file.path(rwldir, paste(str_replace(ids[[i]], ' ', '_'), 'undated.rwl', sep='_'))
...
# Create dataframes for dated and undated cores
lapply(runseq, tucson.wrapper, datesplit, sitespp.ids, rwl.headers)
