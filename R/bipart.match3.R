...
#' @param return.matches logical. Specify whether to return raw matching data or summary statistics. If T, a dataframe with raw matching data is returned for downstream analysis. If F, summary statistics are returned. Default is F.
#' @param plotdir character. Destination directory path for figures produced when computing summary statistics. Default is F. Only relevant if return.matches is F.
...
bipart.match3 <- function(runid, lasset, obset, return.matches=F, plotdir=F) {
  ...
  # If argument `return.matches` is T,
  # then end here and return the dataframe containing raw matching data

  if(return.matches==T) {

    # Bind matches to observed dataframe
    df.matched.a <- left_join(df.paired[df.paired$src==0,], match.1, by=c('treeID'='obs')) %>%
      dplyr::select(pair_id,
                    src,
                    treeID,
                    pred,
    ...
    # Bind matches to modeled dataframe
    df.matched.b <- right_join(df.paired[df.paired$src==1,], match.1, by=c('treeID'='obs')) %>%
      left_join(df.paired, by='treeID') %>%
      dplyr::select(pair_id,
                    treeID=treeID.y,
                    obs=treeID,
                    Zobs=Z,
    ...
    # Rowbind a,b dataframes above and clean
    df.matched <- bind_rows(df.matched.a, df.matched.b) %>%
      dplyr::select(src,
                    pair_id,
                    treeID,
                    obs,
