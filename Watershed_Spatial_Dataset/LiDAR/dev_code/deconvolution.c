...
SEXP py_SpectrumDeconvolution(SEXP py_source, SEXP py_response,
                                      SEXP py_numberIterations,
                                      SEXP py_numberRepetitions, SEXP py_boost )
{

  double *source = REAL(py_source);
  double *response=REAL(py_response);
  int ssize=LENGTH(py_source);
  int numberIterations=INTEGER(py_numberIterations)[0];
  int numberRepetitions=INTEGER(py_numberRepetitions)[0];
  double boost=REAL(py_boost)[0];
  SEXP f;
