...
```{r include=F}
source('inst/examples/species.mapping.R')
```
...
crowns_ras <- lapply(chm.smooth, mcws, treetops = stem.sf, minHeight = 5)

# Plot CHM
crowns_poly <- lapply(chm.smooth, mcws, treetops = stem.sf, minHeight = 8, format='polygons')
...
canopy.filter.e <- unlist(lapply(1:nrow(stem.seg), \(i) {
  
  # if(stem.seg[i,]$Zpred < quantile(stem.seg$Zpred, .25)) {
  if(stem.seg[i,]$Zpred < 8) {
  tst <- 0 
  } else {
    tst <- 1
  }
...
# Apply canopy filter to buffered stems
stem.filt.e <- stem.seg[canopy.filter.e,]
