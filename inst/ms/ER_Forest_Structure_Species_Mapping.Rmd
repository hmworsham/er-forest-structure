...
canopy.filter.a <- unlist(lapply(1:nrow(stem.buff), \(i) {
  if(stem.buff[i,]$Zpred < quantile(stem.buff$Zpred, .9)) {
    if(length(stem.within[[i]])<=1) { 
      tst <- stem.buff[i,]$Zpred > 0.9 * stem.buff[stem.overlap[[i]],]$Zpred &
        !any(stem.buff[stem.overlap[[i]],]$Zpred >= quantile(stem.buff$Zpred, .8))
      tst <- prod(tst)
    } else { 
      tst <- 0 }
  } else {
...
stem.filt.e <- stem.seg %>%
  group_by(Site_Name) %>%
  filter(Zpred >= quantile(Zpred, 0.5)) %>%
  ungroup() %>%
  mutate(Crown_Area=st_area(.))
...
### Confusion matrix
```{r}
stems.comp6 <- get.spp(sp.class, stem.filt.e, sp.codes)
