...
lc <- lascat[825,]
chm.pitfree.025 <- rasterize_canopy(lc, 0.5, p2r(.2), pkg='terra')
...
crowns_poly <- mcws(stem.sf.crp, chm.smooth, minHeight=3, format='polygons')
plot(crowns_ras, col='blue')

stem.seg <- st_join(st_as_sf(crowns_poly), stem.sf, by=c('treeID'))
