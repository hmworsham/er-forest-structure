...
datadir <- file.path(scrdir, 'neon_las_gaps')
geredir <- file.path(scrdir, 'geolocated_returns')
outdir <- file.path(scrdir, 'neon_las_gaps_normalized')
dir.create(outdir)
...
dtm <- terra::rast(file.path(scrdir, 'dtm_mosaic.tif'))
...
opt_output_files(lascat) <-  paste0(outdir, '/las_norm_missing_fp_{XLEFT}_{YBOTTOM}')
