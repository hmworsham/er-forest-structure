...
## Plot
ggplot(allom.val, aes(x=DBH, y=DBH_pred, color=dens)) +
  geom_point() +
...
mclapply(treefiles, \(x) {
  ...
  tf$DBH_est <-exp(predict(m2, tf))*exp(summary(m2)$sigma^2/2)
