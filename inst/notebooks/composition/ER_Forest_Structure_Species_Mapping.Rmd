...
```{r include=F}
source(file.path('.', 'inst', 'notebooks', 'composition', 'species.mapping.R'))
```
...
2. 
  A. Define a buffer of radius $r$ around each tree stem object, approximating crown area.
  B. Segment trees using marker-controlled watershed segmentation algorithm to determine crown area.
...
```{r}
st_write(stems.comp7, file.path(config$data$int, 'spp_class_data.csv'), layer_options = "GEOMETRY=AS_XY")
```
